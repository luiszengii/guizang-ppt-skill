...
**本 fork 是 guizang-tutorial-skill**——教程/报告/学习笔记模式。下面新增了「P0 · 教程模式」section，**所有教程产出必须先过这个，再过原 PPT 的视觉规则**。

---

## 🔴 P0 · 教程模式信息密度自检（本 fork 新增,优先级最高）

教程产出最常见的失败模式是「保留了 PPT 的视觉品味,但产出像 PPT 一样信息稀疏」——单段一句话、大留白、章节之间空空荡荡。这违反本 skill 的核心契约。

### 0-T1. 总字数硬底线

**现象**:打开生成的 HTML,内容刷一下就读完了,跟参考的 `~/Desktop/学习笔记/vercel-preview-原理.html` 差一个数量级。

**判定**:
```bash
wc -m ~/Desktop/学习笔记/<文件>.html
```
HTML 标签也算字符,所以总字符数 < 4000 基本对应正文 < 1500 字,这种产出**必须打回重写**,不准交付。

**做法**:
- 每节正文段落 ≥ 100 字(hero/章节封例外)
- 不允许「一段一句话」——参考 vercel-preview-原理.html 里每节都有 2-4 段完整解释 + 表格/SVG/对比
- 如果素材确实不够 1500 字,先告诉用户「素材不足,需要补充或者放弃做长报告」,不要强行写水分

### 0-T2. 章节数 + 内联视觉元素硬底线

**现象**:整篇只有 2-3 个章节,或者全是纯文字没有任何图表。

**判定**:
```bash
grep -c '<section class="slide' ~/Desktop/学习笔记/<文件>.html  # ≥ 5
grep -c '<svg' ~/Desktop/学习笔记/<文件>.html                    # ≥ 2
```

**做法**:
- 内容章节 ≥ 5 个(不含 hero/开场 + 结尾)
- 内联 SVG 图示 + 数据表合计 ≥ 2 个
- 数据/对比/流程优先用 SVG 或 `<table>`,不要全堆文字

### 0-T3. 禁止 PPT 病:单段一句话 + 大留白

**现象**:沿用了原 PPT 的「一句金句独占一页」 layout(比如 Layout 8 大引用、S09 Statement),但在教程里这就是空白页。

**做法**:
- 教程模式下,Layout 8/Big Quote、S09/Dot Matrix Statement、S03/Split Statement 这类「statement 版式」**只允许在 hero 章节或最后总结章节出现一次**
- 中间正文章节不要用这些版式
- 如果想用 statement 强调一个观点,做成「statement + 紧跟一段 100 字以上解释」的组合

### 0-T4. 不能为了视觉留白删信息

**现象**:为了让某节看起来"干净 / 留白漂亮 / 像 Monocle",删掉了 30% 的解释文字。

**做法**:
- 信息塞不下就换版式,不要删句子
- 杂志风用 Layout 5(图片网格)/10(图文混排)/6(Pipeline) 这类高密度版式,瑞士风用 S04(Six Cells)/S15(Matrix)/S16(Multi-card Brief)/S20(Stacked Ledger) 这类网格密集版式
- 实在塞不下就**多加一个章节**,而不是删

### 0-T5. 输出位置 + 命名约定

**现象**:产出文件放到了项目目录或 `/tmp/`,或者文件名用了英文/没主题描述。

**做法**:
- 输出路径必须是 `~/Desktop/学习笔记/<主题>-<要点>.html`
- 文件名中文连字符,例:`vercel-preview-原理.html` / `react-server-components-为什么.html` / `cap-定理-讲清楚.html`
- 不要存到 `~/.agent/`、`/tmp/`、项目子目录、`./out/` 等位置

---

## 🔴 P0 · 一定不能犯的错(PPT 视觉规则,教程模式继续适用)
